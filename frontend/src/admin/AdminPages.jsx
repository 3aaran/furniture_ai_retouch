...
function Dashboard({setMsg}){
  ...
  return <div className="stack">
    ...
    <section className="panel"><h2>AI消耗分布</h2><div className="bars">{(stats?.ops||[]).map(o=><div key={o.operation}><span>{featureName[o.operation]||o.operation||'未知功能'}</span><i style={{width:Math.min(100,Number(o.count)*12)+'%'}}/><b>{o.count} 次 / {o.quota} 额度</b></div>)}</div></section>
  </div>;
}
...
function AiConfig({setMsg}){
  ...
  return <section className="panel settings aiConfigPageV2"><h2><BrainCircuit/> AI 模型配置</h2>{!ai?<div className="empty">AI 配置加载中...</div>:<><div className="aiConfigBoxV2"><h3>全局模型服务</h3><div className="grid2"><label>模型服务<ModelSelect value={modelOptionValue(findModelOption(ai.providerConfig?.provider,ai.providerConfig?.defaultModel))} onChange={v=>updateAiModelSelection(setAi,v)}/></label><label>API Base URL<input value={ai.providerConfig?.baseUrl||''} onChange={e=>updateProvider('baseUrl',e.target.value)}/></label><label>API Key<input placeholder={ai.providerConfig?.apiKeyMasked||'保存后自动脱敏显示'} value={ai.providerConfig?.apiKey||''} onChange={e=>updateProvider('apiKey',e.target.value)}/></label><label>API Path<input value={ai.providerConfig?.defaultApiPath||''} onChange={e=>updateProvider('defaultApiPath',e.target.value)}/></label></div><label className="check"><input type="checkbox" checked={!!ai.providerConfig?.enabled} onChange={e=>updateProvider('enabled',e.target.checked)}/> 启用 AI 生成功能</label></div><div className="aiConfigBoxV2"><h3>功能模型映射</h3><div className="aiFeatureTableV2"><div className="aiFeatureHeadV2"><b>功能</b><b>启用</b><b>Provider</b><b>Model</b><b>API Path</b></div>{(ai.features||[]).map((f,i)=><div className="aiFeatureRowV2" key={f.featureKey}><span>{f.featureName}<small>{f.featureKey}</small></span><input type="checkbox" checked={!!f.enabled} onChange={e=>updateFeature(i,'enabled',e.target.checked)}/><input readOnly value={f.provider||''}/><ModelSelect value={modelOptionValue(findModelOption(f.provider,f.modelName))} onChange={v=>updateFeatureModelSelection(setAi,i,v)}/><input value={f.apiPath||''} onChange={e=>updateFeature(i,'apiPath',e.target.value)}/></div>)}</div></div><button className="submit" onClick={save}>保存 AI 配置</button></>}</section>;
}
