...
describe('workflow UI contract',()=>{
  it('integrates real admin paths and navigation',()=>{
    assert.match(read('../../App.jsx'),/parseWorkflowPath/);
    assert.match(read('../../App.jsx'),/WorkflowAdminApp/);
  ...
  it('provides all three-node configuration and toolbar controls',()=>{
    const panel=read('./NodeConfigPanel.jsx'),editor=read('./WorkflowEditorPage.jsx');
    ...
    for(const text of['保存','运行测试','返回','工作流创建'])assert.match(editor,new RegExp(text));
  ...
  it('loads the balanced three-panel workflow styles',()=>{
    ...
    assert.match(css,/\.workflowAdminTopbar\{[^}]*margin:0/);
