...
import{adminNav,adminNavGroups,Dashboard,Applications,Merchants,AiConfig,Resources,SettingsPage,AdminLogs,Feedbacks,Announcements,RedeemCodes}from'./admin/AdminPages.jsx';
...
function Shell({me,setMe}){
  ...
  const initial=pageFromHash() || (me.role==='SYSTEM_ADMIN'?'dashboard':'workbench');
  ...
  function go(k){
    const next=pageKeys.has(k)?k:(me.role==='SYSTEM_ADMIN'?'dashboard':'workbench');
    setPage(next);
    setMenu(false);
    setNavDrop(null);
  ...
  function adminGroupActive(g){
    return g.items.some(([k])=>k===page);
  }
  ...
  return <div className="topApp" onClick={()=>setNavDrop(null)}>
    <header className="topbar">
      ...
      <nav className="topNav" onClick={e=>e.stopPropagation()}>
          ...
          <div className="navGroup" key={g.key}>
            ...
            {navDrop===g.key&&
              <div className="navDropdown">
                {g.items.map(([k,t,I])=>
                  <button key={k} className={page===k?'active itemActive':''} onClick={()=>go(k)}>
                    <I size={17}/>{t}
                  </button>
                )}
              </div>
        ...
        {!isAdmin&&
          <button className="iconNav" title="问题反馈" onClick={()=>setFeedbackOpen(true)}>
      ...
      <div className="topRight" onMouseEnter={openProfileMenu} onMouseLeave={closeProfileMenuSoon}>
        <button className="quotaPill" onClick={()=>go('quota')}>
          <WalletCards size={17}/>{me.quota} 算力
        </button>

        <button className="avatarBtn" type="button">
          <span>{(me.displayName||me.phone||me.username||'用').slice(0,1)}</span>
          <div>
            <b>{me.displayName}</b>
            <small>{roleName[me.role]}</small>
          </div>
        </button>
        ...
        {menu&&
          <div className="profileMenu">
            ...
            {isAdmin?
              <button onClick={()=>go('redeem')}><Ticket size={18}/>兑换码创建</button>
              :
              <button onClick={()=>{setRedeemOpen(true);setMenu(false)}}><Ticket size={18}/>礼品卡兑换</button>
            }
            <i/>
    ...
    <main className="topMain">
      {!(page==='workbench'&&!isAdmin)&&
        <div className="pageHead">
          <h1>{nav.find(n=>n[0]===page)?.[1]||({profile:'个人中心',quota:'额度明细',redeem:'兑换码创建',feedbacks:'问题反馈'}[page]||'管理页面')}</h1>
          <p>{isAdmin?'平台管理员可管理运营、审核、商家、模型、资源、配置、日志、反馈和公告。':'当前页面会根据门店角色展示可访问的功能，未授权入口不会出现在导航栏中。'}</p>
        </div>
      }

      {msg&&createPortal(
        <div className={`globalToastV2 ${toastKind==='error'?'error':'success'}`}>
