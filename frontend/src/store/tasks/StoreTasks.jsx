...
export default function StoreTasks({me,setMsg,TaskDetailModal,goPage}){
  ...
  return <div className="adminLogPage">
    ...
    <section className="panel">
      ...
      {/* </Toolbar> */}
      <div className="aiTaskGrid">{taskItems.map(i=><article className="taskCard" key={i.id}><div className="taskImg" onClick={()=>open(i.id)}><img src={imageViewUrl(i)}/><b>{getFeatureDisplayName(i.featureKey||i.operation||i.kind,'AI任务')}</b></div><div className="taskMeta"><strong>{i.userName||'-'}</strong><span>{fmt(i.createdAt)}</span><small>编号：{i.id}</small></div><div className="taskActions"><button onClick={()=>open(i.id)}><Eye size={16}/>详情</button><button onClick={()=>window.open(`${API}/api/images/${i.id}/download?token=${token()}`,'_blank')}><Download size={16}/>下载</button></div></article>)}</div>
