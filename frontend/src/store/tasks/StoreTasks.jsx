...
import{req,fmt,usePaged,Pagination,Toolbar,imageListUrl,fallbackToOriginalImage,openImageDownload}from'../../appShared.jsx';
...
export default function StoreTasks({me,setMsg,TaskDetailModal,goPage}){
  ...
  return <div className="adminLogPage">
    <section className="pageHero"><div><h1>历史任务</h1><p></p></div>
    {/* <button className="primary" onClick={batch}><Download size={17}/>批量下载</button> */}</section>
    <section className="panel">
      {/* <Toolbar onSearch={()=>setQuery(q=>({...q,page:1}))} onExport={batch}> */}
        <input placeholder="任务编号 / 功能 / 用户" value={query.keyword} onChange={e=>setQuery({...query,keyword:e.target.value})}/>
      ...
      {/* </Toolbar> */}
      <div className="aiTaskGrid">{taskItems.map(i=><article className="taskCard" key={i.id}><div className="taskImg" onClick={()=>open(i.id)}><img src={imageListUrl(i)} onError={e=>fallbackToOriginalImage(e,i)} loading="lazy" decoding="async"/><b>{getFeatureDisplayName(i.featureKey||i.operation||i.kind,'AI任务')}</b></div><div className="taskMeta"><strong>{i.userName||'-'}</strong><span>{fmt(i.createdAt)}</span><small>编号：{i.id}</small></div><div className="taskActions"><button onClick={()=>open(i.id)}><Eye size={16}/>详情</button><button onClick={()=>openImageDownload(i,setMsg)}><Download size={16}/>下载</button></div></article>)}</div>
