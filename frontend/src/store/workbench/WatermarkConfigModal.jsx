  ...
  useEffect(()=>{
    ...
    req('/api/watermark/settings')
      .then(d=>{
        ...
        setConfig({...defaultConfig,...(d.config||{}),mode:'text',enabled:d.enabled!==false});
      })
          ...
          <div className="watermarkBody">
            <section className="watermarkControls">
              ...
              <div className="watermarkSliderGrid">
                ...
                <label>旋转 <b>{config.rotate}°</b><input type="range" min="-45" max="45" value={config.rotate} onChange={e=>update('rotate',Number(e.target.value))}/></label>
                {config.style==='tile'&&<label>间距 <b>{config.gap}</b><input type="range" min="100" max="420" value={config.gap} onChange={e=>update('gap',Number(e.target.value))}/></label>}
