...
function Workbench({me,setMe,setMsg,goPage,TaskDetailModal}){
      ...
      return <>
        ...
        <div className="wbResourceGrid">
          ...
          {resourceItems.map(r=><button key={r.scope+r.id} className={selectedResource===String(r.id)?'wbResourceCard active':'wbResourceCard'} onClick={()=>setSelectedResource(String(r.id))}>
            {r.imageUrl?<img src={listImgSrc(r)} alt={r.name} onError={e=>fallbackToOriginalImage(e,r)} loading="lazy" decoding="async"/>:<div className="wbResourcePlaceholder">{resTypeName[r.resourceType]}</div>}
            <b>{r.name}</b>
  ...
  function renderStudioReferenceUpload(){
  ...
  function renderStudioLightControls(){
    return <div className="wbStudioLightCard">
      <div className="wbStudioPanelTitle">环境光效</div>
      <label>
        <span>强度</span><b>{studioLight.strength}%</b>
        <input type="range" min="0" max="100" value={studioLight.strength} onChange={e=>setStudioLight(v=>({...v,strength:Number(e.target.value)}))}/>
      </label>
      <label>
        <span>色温</span><b>{studioLight.colorTemp}K</b>
        <input type="range" min="2500" max="7500" step="100" value={studioLight.colorTemp} onChange={e=>setStudioLight(v=>({...v,colorTemp:Number(e.target.value)}))}/>
      </label>
    </div>;
  }
  ...
  function renderStudioControlPanel(){
    return <>
      ...
      />
      {renderStudioReferenceUpload()}
      {renderStudioLightControls()}
