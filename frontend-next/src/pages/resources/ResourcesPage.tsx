...
export function ResourcesPage() {
  ...
  const filterSheet = (
    <aside className={`assetFilterSheet ${modal === 'filter' ? 'isOpen' : ''}`.trim()} aria-hidden={modal !== 'filter'}>
      <div className="assetFilterHead assetFilterHero">
        ...
        <button type="button" aria-label="关闭高级筛选" onClick={closeModal}><AppIcon name="close" /></button>
      </div>
  ...
  return <div className="assetPage">
    ...
    {modal === 'upload' && <aside className="assetActionSheet isOpen"><div className="assetFilterHead"><div><span>上传资产</span><b>上传图片并保存为资产</b></div><button type="button" aria-label="关闭上传面板" onClick={closeModal}><AppIcon name="close" /></button></div><div className="assetFormGrid"><label><span>选择图片</span><input type="file" accept="image/*" multiple onChange={handleUploadFiles} /></label><label><span>统一名称（可选）</span><input value={uploadName} onChange={(event) => setUploadName(event.target.value)} placeholder="不填则使用文件名" /></label>{categoryFields}</div>{notice && <p className="assetNotice">{notice}</p>}<div className="assetFilterActions"><button type="button" onClick={closeModal}>取消</button><button type="button" disabled={uploadBusy} onClick={uploadAssets}>{uploadBusy ? '上传中...' : `上传 ${uploadFiles.length || ''}`}</button></div></aside>}

    {modal === 'batch-category' && <aside className="assetActionSheet isOpen"><div className="assetFilterHead"><div><span>批量分类</span><b>已选择 {selectedItems.length} 项资产</b></div><button type="button" aria-label="关闭批量分类" onClick={closeModal}><AppIcon name="close" /></button></div><div className="assetFormGrid">{categoryFields}</div>{notice && <p className="assetNotice">{notice}</p>}<div className="assetFilterActions"><button type="button" onClick={closeModal}>取消</button><button type="button" disabled={assetBusy} onClick={applyBatchCategory}>{assetBusy ? '处理中...' : '变更分类'}</button></div></aside>}

    {modal === 'batch-delete' && <aside className="assetActionSheet isOpen"><div className="assetFilterHead"><div><span>批量删除</span><b>确认删除 {selectedItems.length} 项资产</b></div><button type="button" aria-label="关闭批量删除" onClick={closeModal}><AppIcon name="close" /></button></div><p className="assetDangerText">删除后这些资产将从资产库移除，该操作会调用后端删除接口。</p>{notice && <p className="assetNotice">{notice}</p>}<div className="assetFilterActions"><button type="button" onClick={closeModal}>取消</button><button className="danger" type="button" disabled={assetBusy} onClick={applyBatchDelete}>{assetBusy ? '删除中...' : '确认删除'}</button></div></aside>}
