...
test('history task detail uses old compare preview pattern', () => {
  ...
  assert.match(historySource, /<img src=\{imageUrl\(item\)\}/);
  assert.match(historySource, /href=\{fullImageUrl\(item\) \|\| imageUrl\(item\)\}/);
  ...
  assert.match(compareSource, /任务对比详情/);
  assert.match(compareSource, /产品原图/);
  ...
  assert.doesNotMatch(compareSource, /taskPreview/);
  assert.doesNotMatch(compareSource, /taskInfo/);
  assert.doesNotMatch(compareSource, /taskCompare/);
  assert.doesNotMatch(compareSource, /compareHead/);
  assert.doesNotMatch(compareSource, /额外要求/);
  assert.doesNotMatch(compareSource, /promptBox/);
  assert.doesNotMatch(compareSource, /infoRows/);
  assert.match(compareCss, /compareDetailOverlay/);
  assert.match(compareCss, /compareDetailImages/);
  assert.match(compareCss, /compareDetailPanel/);
  assert.match(compareCss, /var\(--bg-page\)/);
  assert.match(compareCss, /object-fit: contain/);
  assert.doesNotMatch(compareCss, /#[0-9a-fA-F]{3,8}/);
  assert.doesNotMatch(compareCss, /compareDetailStats/);
  assert.doesNotMatch(compareCss, /taskPreview/);
  assert.doesNotMatch(compareCss, /taskInfo/);
  assert.doesNotMatch(compareCss, /taskCompare/);
  assert.doesNotMatch(compareCss, /promptBox/);
  assert.doesNotMatch(compareCss, /infoRows/);
  assert.doesNotMatch(operationsCss, /taskPreviewOverlay/);
  assert.doesNotMatch(operationsCss, /taskMobileActionBar/);
