...
export function UserActionModal({ modal, close, setNotice, refresh }: { modal: UserModalState; close: () => void; setNotice: (value: string) => void; refresh: () => void }) {
  const [form, setForm] = useState<Row>(() => modal?.type === 'edit-user' ? { displayName: modal.item.displayName || '', phone: modal.item.phone || '', password: '' } : modal?.type === 'recharge-user' ? { amount: 10 } : { displayName: '', phone: '', role: 'STAFF', quota: 0, password: '123456' });
  ...
  return (
    <div className="opModalMask" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <section className="opModal" role="dialog" aria-modal="true">
        <header><div><span>{modal.type === 'create-user' ? '创建用户' : modal.type === 'edit-user' ? '编辑用户信息' : '用户充值'}</span><b>门店工作台</b></div><button type="button" onClick={close}><AppIcon name="close" /></button></header>
