...
- 平台管理员页面覆盖旧 `frontend` 已提供的管理功能：运营概况、申请审核、商家管理、AI 日志、问题反馈、系统资源、模型配置、系统配置、公告、兑换码和工作流。
...
- 新建 `pages/admin/`，按页面拆分，使用共享的分页、状态展示和简单表格组件。
- 新建 `services/admin.api.ts`，集中封装管理员 API；普通门店 API 保持在现有服务内。
- 在路由层实现声明式角色守卫，在 `AppShell` 中根据角色生成普通用户导航，避免页面内部散落角色判断。
