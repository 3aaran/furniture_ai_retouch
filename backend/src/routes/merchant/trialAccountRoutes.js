...
export async function createTrialAccount(req, res, {
  ...
  try {
    ...
    await conn.query(
      'INSERT INTO users(id,merchant_id,phone,username,display_name,company_name,password_hash,role,quota_balance,status,storage_limit_bytes,trial_expire_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,DATE_ADD(NOW(), INTERVAL ? HOUR))',
      [
        id,
        req.user.merchant_id,
        null,
        account,
        String(displayName || `体验用户_${account.slice(-4)}`).trim(),
    ...
    if (amount > 0) {
      ...
      await conn.query(
        'INSERT INTO quota_logs(id,merchant_id,related_user_id,operator_user_id,amount,type,balance_after,remark) VALUES(?,?,?,?,?,?,?,?)',
        [uuid(), req.user.merchant_id, id, req.user.id, amount, 'MANUAL_ADJUST', Number(targetBalance?.quota_balance || 0), '分配算力给体验账号']
      );
    }
